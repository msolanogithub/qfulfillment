...
function sectionForBox(ctx, box) {
  ...
  const tableBlock = {
    table: { widths: ['*', 60, 70], body: bodyRows },
...
function groupItemsByBox(itemsData) {
  ...
  itemsData.forEach(i => {
    ...
    if (!items[orderItemId]) {
      items[orderItemId] = { name: i.orderItem.shoe.title, total: 0, sizes: [] };
    }
...
function allocateGroupedItemsToBoxes(groupedItems, unitsPerPackage) {
  ...
  function addToBox(box, itemName, sizeKey, qty) {
    let ref = box.references.find(r => r.name === itemName);
    if (!ref) {
      ref = { name: itemName, sizes: {} };
  ...
  for (const id of orderItemIds) {
    ...
    for (let s of sizesArr) {
      ...
      while (qty > 0) {
        ...
        addToBox(currentBox, itemName, sizeKey, take);
        qty -= take;
      }
    }
  }
